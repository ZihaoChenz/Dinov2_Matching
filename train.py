...
for name, param in model.named_parameters():
    ...
    if '11.mlp.fc1' in name or '11.mlp.fc2' in name:
        param.requires_grad = True
    else:
...
for epoch in range(num_epochs):
    model.train()
    for batch_idx, (queries, positives) in enumerate(train_loader):
        batch_size = queries.size(0)
        # 每个query的正样本数量（包括query本身）
        num_samples_per_query = num_pos + 1
        ...
        # 合并特征
        all_features = torch.cat([query_features.unsqueeze(1), positive_features], dim=1)  # [batch_size, num_pos + query, feature_dim]
        all_features = all_features.view(-1, all_features.size(-1))  # [batch_size * 5, feature_dim]
